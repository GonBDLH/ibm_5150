...
use ibm_5150::{hardware::sys::System, util::disassembly::Disassembler};
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            StripBuilder::new(ui)
                ...
                .vertical(|mut strip| {
                    // Add the top 'cell'
                    strip.strip(|builder| {
                        builder.size(Size::relative(0.322))
                               .size(Size::relative(0.12))
                               .size(Size::relative(0.558)).horizontal(|mut strip| {
                            strip.cell(|ui| {
                                self.disassembly_view(ui);
                            });
                            strip.cell(|ui| {
            ...
            ui.vertical_centered_justified(|ui| {
                ...
                egui::Grid::new("mem_grid").striped(true).show(ui, |ui| {
                    ...
                    for i in 0..8 {
                        ...
                        ui.label(egui::RichText::new(&format!("{:05X} | ", addr)).text_style(egui::TextStyle::Monospace));

                        let mut mem_view = String::with_capacity(100);

                        for j in 0..0x10 {
                            mem_view.push_str(&format!(
                                "{:02X} ",
                                self.sys.bus.memory[addr + j]
                            ));
                        }
    ...
    fn disassembly_view(&mut self, ui: &mut Ui) {
        ui.group(|ui| {
            ui.vertical_centered_justified(|ui| {
                ...
                ui.label("");
                let instrs = self.disassembler.run(8, &self.sys.bus.memory, &self.sys.cpu);
