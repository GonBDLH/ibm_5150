...
pub fn debug_82(cpu: &mut CPU) {
    ...
    match get_address(cpu) {
        ...
        0xFE0AD => println!(" - ERROR 1: {:04X?}", ip_queue_lock),
        0xFE3BD => println!(" - EXP ERROR: {:04X?}", ip_queue_lock),
        0xFE809 => println!(" - ROM ERROR: {:04X?}", ip_queue_lock),
        0xFE6BA => println!(" - P_MSG: {:04X?}", ip_queue_lock),
        0xFE5CF => println!(" - ERROR BEEP SUBROUTINE: {:04X?}", ip_queue_lock),
