...
use crate::hardware::cpu_8088::{instr_utils::{Instruction, Direction, decode_mod, decode_reg, AddrMode, OperandType, Operand, Length, Segment}, cpu_utils::{to_u16, sign_extend}, CPU};
    ...
    pub fn run(&mut self, iters: usize, mem: &[u8], cpu: &CPU) -> Vec<(String, String, usize)> {
        ...
        for _ in 0..iters {
            ...
            if let Some(v) = self.cache.get(&ea) {
                ...
                self.decode(&mut instr, mem, op);
                let ea_dif = (((self.cs << 4) + self.ip) & 0xFFFFF) - ea;
                
                instr_vec.push((format!("{}", instr), self.op_string.clone(), ea));
                self.cache.insert(ea, (String::from(format!("{}", instr)), self.op_string.clone(), ea_dif));
            }
        }
...
impl Disassembler {
    ...
    pub fn decode_rm(&mut self, instr: &mut Instruction, mem: &[u8], operand: u8, rm_pos: u8) -> OperandType {
